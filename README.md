...
* ```bOCV[sp,1:nOCV]``` = A 2D matrix (species by covariate) containing the slope (b or beta) parameters for each of our predictor variables. These are calculated independently for each species. We can have as many predictors as we want, so to keep the model tidy I have included this as ```1:nOCV```, where nOCV represents the number of predictors [number(n) of occurence(O) covariates(CV)].
