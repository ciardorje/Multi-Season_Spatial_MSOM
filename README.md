...
* Data:
  * ```Z``` = Our occurence records. A binary 3D array, with dimensions ```nSites x nSps x nYears```, where cell values indicate whether (1) or not (NA; can be considered equivalent to 0, i.e. a non-observation, but NA means we want the model to estimate this value) each species was detected at each site in each year, across all trapping weeks combined.  
  * ```Y``` = Our detection records. A 4D binary array, with dimensions ```nSites x nSps x nYears x max(nTrapWeeks)```, where cell values indicate whether (1) or not (NA) each species was detected at each site, in each trap week, in each year. 
...
Note the use of the ~ symbol. This tells the model this is a stochastic node (a node is just a parameter that the model has been told to estimate). For stochastic nodes we supply a probability density function on the right of the ~, and then the the model will randomly draw a value from the distribution and assign it to the object on the left of the ~.       
